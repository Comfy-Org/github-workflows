...
1. **Path floor** — [`risk-map.v0.json`](risk-map.v0.json): versioned path-glob
   ...
   so a docs file can never cancel a migration in the same PR.
...
3. **Reversibility** — mutates persistent state or deletes data → R3; deletes a
   file under a sensitive class → R3; no green check rollup → R2; green but no
   test file touched → R1; green with tests touched → R0.
...
- **The grading run excludes itself from the check rollup it reads** (its own
  ...
  live grade would floor at R2 as an artifact of the measurement.
...
judge it (editing them at all is R3 by the map's own first rule). Absent files
fall back to the shipped defaults; a present-but-invalid file fails the run
loudly rather than silently grading generic.
...
- `grade-pr-risk.sh` — the grader. `--pr N --repo o/r` grades a live PR;
  ...
  from its collector (BE-5030) — keep the two in sync when either changes.
