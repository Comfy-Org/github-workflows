...
| [`pr-risk.yml`](.github/workflows/pr-risk.yml) | **Advisory PR risk grading (shadow check).** Grades every PR into a tier `R0`..`R3` and syncs one label; nothing is gated, routed, or blocked. Deterministic (`gh` + `jq`, no LLM): worst of path floor, diff provenance, and revertability. | [pr-risk.md](docs/callers/pr-risk.md) |
