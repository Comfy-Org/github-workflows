...
| [`pr-risk.yml`](.github/workflows/pr-risk.yml) | **Non-blocking** PR risk grade — grades a PR's net diff into a tier (`R0` trivial … `R3` needs a careful read) and publishes it three ways so a deep review queue is filterable by how much attention each change needs (`is:pr is:open label:risk:R0`): exactly one `risk:R0`..`risk:R3` **label** at any time (stale tiers removed on every re-grade), an immutable timestamped **Check Run** carrying the tier *and* the reason (the per-change audit artifact), and one **sticky comment** — updated in place, never duplicated per push — with the per-file breakdown, the risk *concentration* ("94% of this diff is R0/R1; the 6% that makes it R3 is these two files, 40 lines"), and a "this grade is wrong" checkbox whose toggling is recorded as a `risk-grade-disputed` label. **Nothing is gated**: the Check Run's conclusion is hardcoded `neutral`, the grader always exits 0, every publish step is best-effort, and no automation consumes the label. **Re-grades on every push** — the caller must include `synchronize` *and* `edited` in its trigger types (`edited` is what catches a **retarget**, which changes the diff without moving the head SHA or firing `synchronize`); a grade that goes stale is worse than no grade. A PR the grader cannot read is published as **unknown** — no tier, no label, never defaulted to `risk:R0`. Ships in **`mode: shadow` by default** (Check Run only), so no consumer sees a reviewer-facing change until its caller opts into `mode: publish`. Two-job security split as in `pr-size.yml`: the grading job runs against PR code with a read-only token, the publishing job holds the bot token and checks out no PR code. The calling job must grant `contents: read` + `checks: write` + `pull-requests: write` (GitHub rejects a shorter grant at startup); `bot_app_id` + `BOT_APP_PRIVATE_KEY` are optional but required for fork PRs. Grading logic + tests live in [`.github/pr-risk/`](.github/pr-risk); the risk map itself is a documented first cut that BE-5507 replaces in place. |
