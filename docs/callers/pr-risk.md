...
| `label_map` | `''` | Rename the five grader-owned labels as `tier=label` pairs. Tier keys are fixed; only the label text is yours. The caller repo's `RISK_LABEL_MAP` variable overrides this input without a PR. |
