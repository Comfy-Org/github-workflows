...
| `pr_numbers` | `''` | Grade **several** PRs by number, comma-separated (`12,15,20`). Takes precedence over `pr_number` when both are set. Targets are graded one at a time and one unreadable PR is reported without abandoning the rest. Pair a long list with a low `wait_for_checks_minutes` — the per-target waits are additive, and the run stops starting new targets once the job's budget is spent, reporting the un-attempted ones by number. Hard-capped at `MAX_TARGETS` (50, not exposed as an input): a longer list is refused outright *before any target is graded*, so an enrollment backfill of more than 50 PRs fails the run red with nothing labeled rather than degrading on time — split it into batches. There is deliberately no `all_open: true`. |
...
| `sticky_comment` | `false` | Publish ONE sticky PR comment — a single visible line (the tier, the axis that decided it and that axis's reason), with the formula, the per-axis reasons, the map versions and the caveats folded into a collapsed `<details>` and a "this grade is wrong" checkbox below it. Created once and updated in place, so N pushes leave one comment — with two bounds worth stating. The marker lookup scans only the newest 10 pages of 100 comments (`STICKY_MAX_PAGES`), so on a PR chatty enough to bury the sticky past ~1,000 comments it reads as absent: the next grade posts a DUPLICATE with `disputed=0`, which can also drop an already-applied `risk-grade-disputed` label while the reviewer's box is still ticked. And the upsert is list-then-create / read-then-PATCH with no atomicity — a `pr_numbers` batch and an event run cannot share a per-PR concurrency group, so two first grades can both see no marker and both post, and a tick landing between the read and the PATCH is overwritten. Ordinary single-run pushes behave exactly as described; the guarantee is weaker than it sounds at the edges. **Defaults to false on purpose:** a bot comment on every PR is a visible behaviour change, so it is a per-consumer decision, and a caller that does not opt in behaves byte-identically to before this input existed. Ticking the checkbox applies `risk-grade-disputed` on the next grade — a machine-maintained mirror, distinct from the human-owned `risk-dispute:*` overrides — and needs no permission beyond the `pull-requests: write` already granted. |
| `check_run` | `false` | Publish a Check Run on the PR's head commit carrying the tier and the reason: the immutable, timestamped, commit-attached record a mutable label cannot be. Its conclusion is hardcoded `neutral`, so it can never fail a PR even if a repo later marks it required. **Leaving it false does not excuse the `checks: write` grant** — see "Grant the whole union" above; the grant is validated at startup, the input only decides whether the `publish-check` job runs. **A Check Run also requires `enabled: true`.** `publish-check` is gated on `enabled` as well as on this input, while `grade` deliberately admits a `workflow_dispatch` regardless of it — so on exactly the staged-rollout path the `enabled` row advertises, a manual dispatch against a repo still switched off grades, labels and (with `sticky_comment`) comments, but publishes no Check Run. Evaluating the Check Run before switching the repo on therefore means making `enabled` true for that run — `vars.RISK_CONFIG` = `{"enabled": true}` (no PR) or `enabled: true` in the caller's `with:` — because grading alone will not publish one. |
| `check_name` | `PR risk (advisory)` | Name of the published Check Run. Only used when `check_run` is true. **Do not reuse the name of a required check.** Nothing validates this value, and the conclusion is hardcoded `neutral` — which branch protection treats as satisfied — so pointing it at a required check's name lets this advisory run stand in for the real merge gate. Keep it distinct and obviously advisory. |
