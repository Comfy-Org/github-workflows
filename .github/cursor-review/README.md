...
| [`fence-diff.py`](fence-diff.py) | Wraps the reviewed diff (and the incremental hunks) in `=== BEGIN/END DIFF <nonce> ===` fences carrying a per-run nonce minted in the `diff-size` job. The diff is attacker-authored PR bytes, so static literal fences are not a control; the nonce is what a PR cannot forge. Copies the diff through **byte for byte** — it never defangs or normalizes the payload. |
