...
#                  (the Cloud Code Bot app token generated by the entrypoint) AND
#                  able to READ this repo's Actions variables — i.e. minted with
#                  NO `permission-*` downscoping inputs, since the app-permissions
#                  API schema has no `variables` key to ask for (BE-6477).
...
if [[ -z "${CALLERS_JSON+x}" ]]; then
  ...
  VAR_ERR=$(mktemp)
  if ! CALLERS_JSON=$(gh api "repos/${GITHUB_REPOSITORY}/actions/variables/${VAR_NAME}" --jq '.value' 2>"$VAR_ERR"); then
    if grep -qi 'HTTP 404' "$VAR_ERR"; then
      # No such variable == an empty roster, spelled the same way an empty
      # variable is, so the ALLOW_EMPTY block below keeps its EXACT semantics:
      # a clean no-op for a fleet seeded empty, and the loud clobber-detecting
      # error for a fleet that must always have callers (auto-label,
      # cursor-review, detect-unreviewed-merge).
      CALLERS_JSON=""
    elif grep -qi 'HTTP 403' "$VAR_ERR"; then
      rm -f "$VAR_ERR"
      echo "::error::App token cannot read Actions variables — grant the Cloud Code Bot App repository permission 'Variables: read' and approve it on the Comfy-Org installation (BE-6477)."
      exit 1
    else
      # Echo the REASON only, never the value (the value went to stdout; this is
      # stderr). Sanitized the same way the stale-pin warning is: the run logs of
      # this public repo are a workflow-command sink, so a `\r::` in an error body
      # could otherwise inject a command of its own.
      VAR_REASON=$(tr -d '\r' <"$VAR_ERR" | sed -E 's|::|:|g' | tr '\n' ' ' | sed -E 's|[[:space:]]+$||')
      rm -f "$VAR_ERR"
      echo "::error::Could not read the ${VAR_NAME} Actions variable: ${VAR_REASON}"
      exit 1
    fi
  fi
  rm -f "$VAR_ERR"
