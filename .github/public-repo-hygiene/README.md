...
| 2 | Internal collaboration-tool links | Notion (`notion.so`/`notion.site`), Slack (`slack.com/archives`, `slack.com/client`, `app.slack.com`), Google Docs/Drive, `app.datadoghq.com`, `posthog.com/project/`, `linear.app`, and `incident-NNN`. Case-insensitive. Public marketing pages on the same hosts (`posthog.com/docs`) are not matched. Each host is anchored on **DNS-label boundaries**, not on `\b`: a preceding dot is a real subdomain edge (`comfy.slack.com`, `www.notion.so` are those services and are matched), while a preceding letter, digit or hyphen makes it a different registrable domain (`fooslack.com`, `evil-posthog.com`, `my-linear.app` are not matched). An explicit **port** is tolerated, so `notion.so:443/` cannot walk past a pattern that requires `/` after the host. On the right, the required `/` after host+port is what rejects a suffix (`notion.so.evil.com/`); `app.slack.com` is the one host-only pattern and carries its own `(?!\.?[A-Za-z0-9-])`, which rejects `app.slack.com.evil.com` while still allowing a sentence-final period, a port and end of line. |
...
and onto DNS-label boundaries and taught them about ports, which removed the lookalike-host false
positives and the `:443` bypass listed above.

- **The scan is line-oriented.** A reference split across two lines is not matched.
- **A host written with a trailing root label is not matched.** `https://notion.so./page` is the
  same host to a resolver, but the category-2 patterns require `/` (or a port then `/`) directly
  after the host, so the extra dot walks past them. This predates the DNS-label anchoring and is
  unchanged by it; obfuscated spellings of the same hosts — percent-encoding (`%2E`), punycode
  (`xn--`), defanging (`notion[.]so`) — are likewise not matched. This is a hygiene guard against
  an accidental paste, not an adversary who is actively hiding a link.
