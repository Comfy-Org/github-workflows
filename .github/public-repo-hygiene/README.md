...
- **The non-GitHub host allowlist is a list, so a new one starts out over-flagging.** Only `huggingface.co` and `hf.co` clear a `Comfy-Org/<name>` reference today. A second model or package host publishing under the same org name is a false positive until it is added to `NON_GITHUB_ORG_NAMESPACE_HOSTS` — the same one-line edit, in the same caller-unreachable file, that adding a repo name is. The path segments between host and org are enumerated for the same reason (it is what keeps the lookback a bounded constant rather than a per-match scan of the whole line), so `huggingface.co/somewhere/Comfy-Org/x` is a finding. Both fail toward flagging, never toward silence.
- **The host has to be the URL's authority, and a userinfo spelling is the accepted cost.** The category-2 left anchor (`_HOST_L`) is not reused here, because its polarity is wrong: those patterns *detect*, where matching too much over-flags, while this one *suppresses*, where matching too much goes silent. `https://evil.example/huggingface.co/Comfy-Org/x` (a path segment spelled like the host) and `https://evil.huggingface.co/Comfy-Org/x` (a subdomain) are therefore both findings. The anchor asks for an authority position — after `://`, after a token-initial `//`, or starting a token — so the one real spelling it turns away is `https://user@huggingface.co/…`, whose host genuinely is allowlisted. That fails toward flagging and no corpus instance exists. A **non-ASCII neighbour** is turned away too, and that one was a genuine hole rather than a trade: the anchor's reject class is matched under `re.ASCII`, so the ASCII spellings alone left every non-ASCII character reading as a boundary and `https://éhuggingface.co/Comfy-Org/x` and `中huggingface.co/Comfy-Org/x` were both silenced — the scheme-bearing spelling included, since the accented character sits between the `://` and the host and defeats that alternative as well. `éhuggingface.co` is a different registrable name in exactly the way `myhuggingface.co` is, so the class now carries the whole non-ASCII range. The cost is the mirror image of the one `_nonascii_tail` takes on the right: a real bare host butted straight against non-Latin prose or a curly quote is a finding, and a separator or a scheme clears it. One residual is documented rather than closed, because closing it costs a real false positive: a host embedded in a **query string** (`?u=huggingface.co/Comfy-Org/x`) still clears, since rejecting `=` would over-flag a bare `VAR=huggingface.co/…` assignment.
...
starts publishing under the org name, add that host to `NON_GITHUB_ORG_NAMESPACE_HOSTS` (and, if it
interposes path segments before the org, to `_NON_GITHUB_HOST_PATH_PREFIXES`) instead.
