...
- **A path segment spelled like an allowlisted host clears.** The left anchor is the DNS-label one category 2 uses, so `https://evil.example/huggingface.co/Comfy-Org/x` reads the segment as the host and goes silent. Same shape and same trade as the category-2 patterns, which match their hosts wherever they appear in a path.
