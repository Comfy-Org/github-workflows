...
and onto DNS-label boundaries, taught them about ports and pinned them to ASCII, which removed the
lookalike-host false positives, the `:443` bypass and the Unicode-case-folding lookalikes listed
above.
...
- **A host written with a trailing root label is not matched by the `/`-requiring patterns.**
  ...
  percent-encoding (`%2E`), punycode (`xn--`), defanging (`notion[.]so`) — are likewise not
  matched. This is a hygiene guard against an accidental paste, not an adversary who is actively
  hiding a link.
- **An internationalized neighbour of a category-2 host reports as that host.** The left anchor is
  ...
  prose, and a missed leak is worse here than an extra finding. The reverse direction is closed —
  the category-2 patterns compile with `re.ASCII`, so Unicode case-folding no longer reads
  `lınear.app/x` or `ſlack.com/archives/x` as the real hosts.
