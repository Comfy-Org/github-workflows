...
- **`app.slack.com` followed by a colon and a dotted number is not matched.** The host-only
  pattern rejects `:443.evil.com` — a port the host continues past — with `:\d+\.[A-Za-z0-9-]`,
  and a dotted version in prose has exactly that shape, so `See app.slack.com:2.5 release notes`
  goes silent. Requiring a *letter* after the dot would recover it and reopen
  `app.slack.com:443.1evil.com`, since a digit-leading DNS label is legal — one miss traded for
  another, in the direction that flags less. Kept as-is and pinned by
  `test_a_dotted_number_after_the_colon_is_a_known_miss` so it cannot drift unnoticed.
...
- **An internationalized neighbour of a category-2 host reports as that host.** The left anchor is
  ...
  prose, and a missed leak is worse here than an extra finding. The reverse direction is narrowed, not closed —
