...
| # | Category | Shape |
|---|---|---|
| 1 | Ticket-style identifiers | `(?<![A-Za-z0-9])[A-Z]{2,6}-[0-9]{2,6}(?![A-Za-z0-9])` — a generic SHAPE, never a list of real internal team keys, so the check itself discloses no internal naming scheme. The boundaries are explicit lookarounds rather than `\b` because `_` is a word character: under `\b` the most ordinary spellings of a ticket id — `BE-1234_design`, `feature/BE-1234_fix`, `FOO_BE-1234` — were all silently missed, and a non-ASCII letter glued to the front (a Cyrillic homoglyph, say) was a way past the check. `_` is deliberately absent from the excluded class (putting it back is just `\b` again), and so is `-` (adding it would stop `AES-128` matching inside `AES-128-CBC` and break the namespace carve-out below). The digits are `[0-9]` rather than `\d`, which also matches Unicode decimal digits: `BE-` followed by seven Arabic-Indic digits took six into the body and found the seventh outside the ASCII boundary class, so it matched where the ASCII `BE-1234567` does not. Common tech acronyms that fit the shape (`SHA-256`, `AES-192`, `WIN-64`, …) are allowlisted; a caller extends that list with the `ticket_allowlist:` input. Every built-in entry has to be one the regex can actually MATCH — 2-6 letters AND 2-6 digits — which a test enforces, because entries such as `UTF-8` or `X-25519` read as coverage while being unreachable; a second test bans the other dead shape, an entry a namespace prefix below already clears (`ISO-8601`, `RFC-3339`), since `_line_findings` never reaches the exact list for those either. Well-known **public identifier namespaces** clear by PREFIX rather than as exact tokens — `CVE-`, `CWE-`, `PEP-`, `RFC-`, `ISO-`, `UTF-` — because `CVE-2021-44228` presents to this regex as `CVE-2021` (the boundary holds against the following hyphen), so a SECURITY.md or a dependency changelog reddened a required check, and an exact carve-out would cost one entry per year prefix and break again each January. That is also why `UTF-16`, `RFC-1123`, `ISO-8601` and `ISO-27001` are not exact entries: their namespace already clears them. None of those is a plausible internal team key. |
...
| 3 | `Comfy-Org/<repo>` references | **Default-deny** against `PUBLIC_COMFY_ORG_REPOS`. Anything else is flagged so a maintainer either scrubs it or adds it once confirmed public. A leading `@` makes the match a CODEOWNERS **team** handle instead, checked against `PUBLIC_COMFY_ORG_TEAMS` — team handles are inherently public on a public repo (GitHub renders CODEOWNERS owners to anyone who can see it), so an allowlisted one is not a leak. That same `@Comfy-Org/<name>` spelling is also how npm and GitHub Packages write a **scope**, so the `@` path accepts either allowlist; the reverse crossing is not allowed, since a bare `Comfy-Org/<name>` is unambiguously a repo path. A trailing `.git` is stripped, because `Foo.git` in a `repository.url` still references the public repo `Foo`, and a trailing `.` is stripped as sentence punctuation on both paths (a GitHub slug can never end in a dot); a reference left naming nothing (`Comfy-Org/.`, `Comfy-Org/.git`) is not a finding. Case-insensitive throughout — GitHub resolves owner names case-insensitively, so `comfy-org/<repo>` is checked exactly like `Comfy-Org/<repo>`, the `.git` suffix is stripped whatever its casing, and allowlist membership is compared casefolded. The repo-name class itself stays ASCII on purpose (the ignore-case flag is scoped to the org segment): a whole-pattern flag would let Unicode case-folding admit `ſ`/`K` into the name and fold them onto allowlisted spellings. Both ends of the match are bounded: the org segment must **start a token** (`NotComfy-Org/x` is a different org, not a reference to this one), and a name the ASCII class could only **partly read** is never cleared — a Unicode letter or a homoglyph dash immediately after the capture is the *rest of the name*, not a boundary, so `Comfy-Org/comfyui‐internal` written with U+2010 would otherwise test `comfyui` against the allowlist and pass while the full private name sat in the tree. Such a reference is reported whole, with its own remedy (rewrite it in ASCII), because "add it to the allowlist" is not the fix for a homoglyph. The npm/Packages crossing is narrowed to a spelling that could actually **be** an npm coordinate — those are required to be lowercase — so `@Comfy-Org/comfyui`, a team named after the repo it owns, does not clear the team allowlist by borrowing the repo one. That narrowing is not enough on its own, so the crossing is additionally **denied in the owner fields of a CODEOWNERS file**: team slugs are lowercase by construction and the org segment resolves case-insensitively, so `@comfy-org/comfy-cli` on an owner line is a real, functional team handle, and an npm coordinate never appears there. Two questions decide where that applies. **Which file** is the three locations GitHub actually reads CODEOWNERS from — `CODEOWNERS`, `.github/CODEOWNERS`, `docs/CODEOWNERS` — with the NAME matched case-insensitively — lowercased, not casefolded, since a Unicode fold is not a case comparison and folds `codeownerſ` onto `codeowners` — (git records whatever casing the author typed, and a file GitHub does not honor still holds a real team name someone wrote as an owner) but the LOCATION not widened: a `tests/fixtures/CODEOWNERS` or a `docs/notes/codeowners` design note has no owner grammar, and parsing its prose as pattern-then-owners turned a line that cleared before into a hard finding on a required check. **Where on the line** is the owner fields — everything after the leading path pattern — because two other things in a real CODEOWNERS file legally carry the same spelling without being handles: a comment naming a package, and a scoped monorepo path pattern such as `/packages/@comfy-org/comfy-cli/**`. A `#` comments out a WHOLE line only, which is all GitHub honors; ending the scanned text at the first `#` anywhere was a one-character bypass of the entire gate (`docs/#archive/** @comfy-org/comfy-cli`), so a trailing `#` now stays inside the span — the over-flag direction. The accepted cost is that a package named in trailing prose (`* @Comfy-Org/comfy-cloud-team  # see @comfy-org/comfy-cli on npm`) is reported; stopping at a whitespace-delimited `#` token would re-open that bypass, so it is pinned by a test rather than narrowed, and the remedy is to reword the comment. A line whose **only** field has **owner shape** (`@user` or `@org/team` — at most one `/`, no trailing `/`, no glob metacharacter) carries no pattern and is read as all owners. Both halves of that test are load-bearing, because GitHub parses field one as the pattern unconditionally: shape stops a root-level scoped pattern such as `@comfy-org/comfy-cli/**` from handing its own name to the deny, and the only-field bound stops the metacharacter-free `@comfy-org/comfy-cli @comfy-org/<team>` — handle-SHAPED, but still a pattern — from doing the same. The rest of the line model tracks GitHub's parser for the same reason: fields are delimited on **ASCII space and tab** (Python's `\s` is Unicode-wide, so U+00A0 was read as indentation and `\xa0# @comfy-org/comfy-cli` passed as a whole-line comment), the file is split into lines on **`\n` alone** (`str.splitlines()` also breaks at `\r`, `\v`, `\f`, `\x85` and U+2028, so `* @comfy-org/<team>\r# @comfy-org/comfy-cli` became a second line starting `#`), and a UTF-8 BOM is skipped before the first-character decisions on **line 1 only** — it survives decoding and is not whitespace, so it otherwise joined the first token and silently disabled the gate there, while discarding it on every line let a mid-file U+FEFF re-open the same bypass. Each of those was a one-invisible-character bypass of the whole gate. The **name** matched case-insensitively but the **directory** exactly: `DOCS/CODEOWNERS` is a path GitHub does not read. A line with no owner fields (blank, comment-only, or a pattern with no owners) is left to the ordinary crossing. Everywhere else the lowercase-narrowed crossing stands, because a lowercase mention in a README, a Dockerfile `npm i` line or a CI script genuinely could be a package coordinate — that residual ambiguity is accepted, since re-denying it would re-open the false-positive class the crossing exists to fix. Finally, the reference has to be a **GitHub** one, because `Comfy-Org/<name>` is not GitHub's namespace alone: Comfy-Org publishes model weights under `huggingface.co/Comfy-Org/`, so the same token there names a Hugging Face repository with no GitHub existence at all — unallowlistable (there is no GitHub repo to confirm public) and unremovable (the URL is the product content telling a user where to download the weights). Two shapes clear. A reference **immediately preceded by** a model host (`MODEL_HOST_PREFIX_RE`: `huggingface.co` or `hf.co`, an optional scheme, an optional port, and one of the routes that put an owner straight after it — `/<owner>/`, `/models|datasets|spaces|collections/<owner>/`, or the API's `/api/…/<owner>/`) is that host's path, not a GitHub one. The left anchor is `_AUTHORITY_L`, deliberately NOT the category-2 `_HOST_L`: that one was written for a **detector**, where matching too much only costs an extra finding, and this is a **suppressor**, where matching too much goes silent — so every gap `_HOST_L` documents as an accepted trade would flip from fail-safe to fail-open. `_AUTHORITY_L` requires a delimiter (or line start) before the host and consumes the scheme separator, so `myhuggingface.co/Comfy-Org/x`, `evil_huggingface.co/Comfy-Org/x`, `evil.huggingface.co/Comfy-Org/x`, `éhuggingface.co/Comfy-Org/x`, `evil.example/huggingface.co/Comfy-Org/x` and `huggingface.co@evil.com/Comfy-Org/x` are all still flagged; the route segments are `(?-i:…)`-scoped so `/MODELS/` and `/Datasets/`, which resolve nowhere, do not clear; and `api/` is not optional on its own, since HF routes no `/api/<owner>/`. Both narrowings fail closed. On the right the prefix has to END exactly where the reference STARTS, so a model-host link earlier on the line cannot reach a later bare reference. And a **markdown link label** whose target is such a URL for the **same name** — `[Comfy-Org/Qwen-Image-Edit_ComfyUI](https://huggingface.co/Comfy-Org/Qwen-Image-Edit_ComfyUI)` — clears too: the label is a bare token preceded by `[`, so no offset the line-level scan produces can reach it, and the link target is what says which namespace it names. Both ends of the label are checked: the reference has to OPEN it (a bare token in prose that merely happens to be followed by `](<matching URL>` is not link text), and the names must match on a WHOLE path segment on the target side, so a label naming one repo over a link to another stays a finding — including when the target's name only *starts* with the label's (`…/Comfy-Org/x‐model` written with U+2010, `…/Comfy-Org/x%2Dother`, or a name running into the destination's length bound). The host list is deliberately **narrow rather than “anything that is not github.com”**: the broad rule would silence `<anything>.<tld>/Comfy-Org/<private-repo>` wholesale, and recognising a host at all would need a fuzzy looks-like-a-hostname test that an ordinary `docs/v1.2/Comfy-Org/notes` path also passes. Bare, `github.com/` and unrecognised-host references are all exactly as strict as they were. |
...
the ticket boundaries exclude only `[A-Za-z0-9]`, so `/`, `-` and `_` are all boundaries and `notes/TEAM-1234/plan.md`, `TEAM-1234-notes.md` and `notes/BE-9012_design.md` all match — the `_` case being one `\b` used to miss, since `_` is a word character.
...
Unicode-case-folding lookalikes listed above. BE-8910 reused those same anchors to make
category 3 host-aware, which removed the `huggingface.co/Comfy-Org/<model>` false positives —
19 of the 29 category-3 findings on `comfy-cli` at the time — without touching the bare or
`github.com/` spellings.

- **The scan is line-oriented.** A reference split across two lines is not matched.
- **Taking the ticket boundaries off `\b` over-flags `SCREAMING_SNAKE` neighbours.** The change is symmetric: making `_` a boundary is what catches `FOO_BE-1234`, and it equally makes any underscore-separated identifier or asset name whose adjacent segment fits the shape a finding — `BATCH_SIZE-16`, `SD_XL-1024`, `CRC-32_table`, `IMG-2024_v2.png` — on the contents, symlink-target and path surfaces alike, none of which matched under `\b`. On a required check the remedies are an exact `ticket_allowlist:` token or `exclude_paths:`, and the latter also stops that subtree's CONTENTS being scanned, so prefer the allowlist. The trade is accepted deliberately: `\b` bought silence on those names by missing the ordinary spellings of a real ticket id, which is the leak this category exists to catch.
...
A `Comfy-Org/<name>` that is **not a GitHub repo at all** takes the other door. A Hugging Face
model (`huggingface.co/Comfy-Org/<model>`) has no GitHub visibility to confirm, so it never belongs
in `PUBLIC_COMFY_ORG_REPOS` — putting it there would allowlist any private GitHub repo that later
took the same name. It is already handled by host awareness; if a *different* non-GitHub host
starts publishing under the org name, add that host (and any route segments it puts before the
owner) to `MODEL_HOST_PREFIX_RE` instead.
