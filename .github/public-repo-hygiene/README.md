...
- **The host has to be the URL's authority, and a userinfo spelling is the accepted cost.** The category-2 left anchor (`_HOST_L`) is not reused here, because its polarity is wrong: those patterns *detect*, where matching too much over-flags, while this one *suppresses*, where matching too much goes silent. `https://evil.example/huggingface.co/Comfy-Org/x` (a path segment spelled like the host) and `https://evil.huggingface.co/Comfy-Org/x` (a subdomain) are therefore both findings. The anchor asks for an authority position — after `://`, after a token-initial `//`, or starting a token — so the one real spelling it turns away is `https://user@huggingface.co/…`, whose host genuinely is allowlisted. That fails toward flagging and no corpus instance exists. Two residuals are documented rather than closed, both because closing them costs a real false positive: a host embedded in a **query string** (`?u=huggingface.co/Comfy-Org/x`) still clears, since rejecting `=` would over-flag a bare `VAR=huggingface.co/…` assignment; and a **non-ASCII neighbour** (`中huggingface.co/…`) still clears in the *bare* spelling only, the category-3 direction of the IDN-neighbour limitation below — rejecting it would over-flag a real bare host written after CJK prose or a curly quote. A scheme-bearing URL is unaffected either way.
